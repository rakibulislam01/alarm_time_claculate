...
class DetectDate:
    ...
    def __init__(self, device_time='0'):
        ...
        if '/' in device_time:
            ...
            self.now = self.now.replace(year=int(cal_time[0]), month=int(cal_time[1]), day=int(cal_time[2]),
                                        hour=int(cal_time[3]), minute=int(cal_time[4]))
            ...
            self.now = self.now.replace(year=int(year), month=int(month), day=int(day), hour=int(hour),
                                        minute=int(minute))
    ...
    def date_time_detect(self, data):
        ...
        data = data.replace('one', '1').replace('two', '2').replace('three', '3').replace('four', '4').replace('five',
                                                                                                               '5').replace(
            'six', '6').replace('seven', '7').replace('eight', '8').replace('nine', '9').replace('zero', '0')
        # =============================================================================
        ...
        if ('hour' in dictionary.keys() or 'minute' in dictionary.keys() or 'second' in dictionary.keys() or
                'day' in dictionary.keys() or 'month' in dictionary.keys()):
            ...
            # =============================================================================
                ...
                if self.alarm_hour >= 24:
                    ...
                    self.alarm_hour == self.now.hour and self.alarm_minute <= self.now.minute and self.alarm_day == self.now.day and self.flag == 1):
                self.alarm_hour += 12
                if self.alarm_hour >= 24:
                    self.text = "Invalid Time! Would you like to tell me the right time again."
                    self.alarm_hour = 1
                ...
                if self.alarm_hour >= 24:
                    ...
                    self.alarm_minute = 1
                    self.status = 2
            alarm = self.now.replace(year=self.alarm_year, month=self.alarm_month, day=self.alarm_day,
    ...
    def millisecond_diff_from_now(self, data):
        alarm = self.date_time_detect(data)
        alarm_sec = time.mktime(alarm.timetuple())
        now_sec = time.mktime(self.start_time.timetuple())
        # calculate second distance from now to alarm
